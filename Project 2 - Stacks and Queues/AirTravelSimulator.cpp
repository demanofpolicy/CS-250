...
{
    cout << "Enter the maximum capacity for the airplane"<< endl;
    int maxCapacity;
    cin >>  maxCapacity;
    ...
    while( m_state != FINISHED)
    {
        ...
        else if(m_state==LINEUP) // else if
        {
            bool lineup;
            lineup = LineUp();
            if(lineup==true)
        ...
        if ( m_state == DISEMBARKING )
        {
            ...
            // call the disembark function
            disembarking = Disembark();
            if ( disembarking==true)
            {
                m_state = FINISHED;
            }
...
bool AirTravelSimulator::LineUp()
{
    ...
    Traveller*travellers = m_pplManager.GetTraveller(random);
    m_airport.LineUp(travellers);
    m_ptrTravellers.PushBack(travellers);
    travellers->state = LINEUP;
    DisplayMessage(travellers,LINEUP);
...
/**
    1. Create a Traveller* pointer and get the next item from the m_airplane with the Disembark() functinon.
...
{
    Traveller*NextPassenger = m_airplane.Disembark();
    NextPassenger->state = FINISHED;
    DisplayMessage(NextPassenger, DISEMBARKING);
